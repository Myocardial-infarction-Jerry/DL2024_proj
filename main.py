...
def get_cross_validation(path_list, fold_num, current_fold):
    ...
    print(
        f'train sample number:{len(train_id)}, val sample number:{len(validation_id)}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m',
                        '--mode',
                        default='train',
                        choices=["train-cross", "train",
                                 "inf", "inf-cross", "train-noval"],
        ...
        if args.mode == 'inf':
            ...
            if args.save == 'no' or args.save == 'n':
                ...
                feature_save_path = os.path.join(
                    feature_dir, f'fold{str(CURRENT_FOLD)}')
                ...
                for i in range(len(test_path)):
                    ...
                    feature_path = os.path.join(
                        feature_save_path, name.split(".")[0])
            ...
            if INIT_TRAINER['net_name'].startswith('res') or INIT_TRAINER['net_name'].startswith('wide_res'):
                ...
                fc_weight_save_path = os.path.join(
                    save_dir, f'fold{str(CURRENT_FOLD)}_fc_weight.npy')
                np.save(fc_weight_save_path, classifier.net.state_dict()[
                        'fc.weight'].cpu().numpy())

            # save as csv
            report_save_path = os.path.join(
                save_dir, f'fold{str(CURRENT_FOLD)}_report.csv')
            ...
            for fold in range(1, FOLD_NUM+1):
                ...
                if INIT_TRAINER['net_name'].startswith('res') or INIT_TRAINER['net_name'].startswith('wide_res'):
                    fc_weight_save_path = os.path.join(
                        save_dir, f'fold{str(fold)}_fc_weight.npy')
                    np.save(fc_weight_save_path, classifier.net.state_dict()[
                            'fc.weight'].cpu().numpy())

                # save as csv
                report_save_path = os.path.join(
                    save_dir, f'fold{str(fold)}_report.csv')
                report_csv_file = pd.DataFrame(cls_report)
                report_csv_file.to_csv(report_save_path)
    ###############################################
